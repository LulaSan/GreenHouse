...
class Linker:
    ...
    def actuator(self,brgh):
        ...
        res =(requests.post(self.server_name + ':' + str(self.port) + "/greenhouse/{greenhouse_id}",json=new_value).text) 
        print(res)
        if self.mode == 1: #automatico
            if self.brgh > self.max_br and self.status == 1:
                ...
                new_value = {'Greenhouses'[ "STATUS_WINDOW": str(0)]} 
                # new_value = {"STATUS_WINDOW": 0}
                greenhouse_id=self.clientID
                res =(requests.post(self.server_name + ':' + str(self.port) + "/greenhouse/{greenhouse_id}",json=new_value).text) 
                print(res)                        
                ...
                new_value = {'Greenhouses'[ "STATUS_WINDOW": str(1)]} 
            ...
            if self.brgh > self.min_br and self.brgh < self.max_br:
                ...
                elif self.cat["Greenhouses"][["GREENHOUSE_ID"]==self.clientID]['STATUS_WINDOW']==0:
                    soggetto.start_pub()
                    message = {"method": "setLight",
                            "params": 0
                            }
...
if __name__ == '__main__':
    ...
    while True:
        for num in range(num_of_greenHouses):
            ...
            print(green_ID)
            connect= Subscriber_get_info_b("controller_brgth",'/p4iot/greenhouses/'+green_ID+'/sensors', '127.0.0.1', 1883)
            ...
            operator = Linker(green_ID,'127.0.0.1', 1883)
